...
# for training. Each file contains one or more records. Each record is a TensorFlow
...
def _write_recordio(f, data):
    """Write a single data point as a RecordIO record to the given file."""
    length = len(data)
...
def _write_numpy_array(f, feature_name, label, arr):
    feature = {'labels': _label_feature(label), feature_name: _string_feature(arr)}
    example = tf.train.Example(features=tf.train.Features(feature=feature))
    _write_recordio(f, example.SerializeToString())


def build_record_file(filename, num_records, dimension, classes=2, data_feature_name='data'):
    """Build a recordio encoded file of TF protobuf Example objects.
    ...
    with open(filename, 'w') as f:
        for i in range(num_records):
            ...
            _write_numpy_array(f, data_feature_name, cur_class, np.random.normal(loc=loc, size=(dimension,)))
